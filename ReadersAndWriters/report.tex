...
First one is to avoid collisions when writing to the variable \lstinline{nr} and the is for \lstinline{nw}.
...
\begin{itemize}
    ...
    \item multiple readers have access, here the first reader aquires and the last one releases the semaphore
\end{itemize}
A writer can only get access if he aquires the semaphore.
The first readers can get access only when he aquires the semaphore.
All further readers can access the data without aquires this semaphore.
They are still safe as the first reader aquired it.
...
As long as a writer is writing or waiting to write, the \lstinline{reader} semaphore is set.
To achieve this we track the number of currently interested writers \lstinline{nw}.
...
Every writer coming later only increases the number of writers (nw).
The semaphore \lstinline{reader} is only released after the last writer is finished (when nw is 0).
...
But if the reader has a higher priority at the semaphore and a very unfair scheduler it could still happen that the writer waits forever.
